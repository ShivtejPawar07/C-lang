...
void get_data()
{
	...
	for(i=0;i<n;i++)
	{
	printf("\nenter process name,arriavl time,cpu burst time");
	scanf("%s %d %d",&p[i].pname,&p[i].AT,&p[i].BT);
	}
}
void put_data()
{
	printf("PROCESSES ARE AS BELOW");
	printf("\nprocess name\t arrival time\tcpu burst time\tfinish time\tstart time");
	for(i=0;i<n;i++)
