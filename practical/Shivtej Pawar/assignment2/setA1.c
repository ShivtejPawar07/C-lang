...
char ss[40],tok1[10],tok2[10],tok3[10];
pid_t pid;
char ch[1],ch1[100];
...
void sep()
{
  ...
  strcpy(tok3,"\0");
   ...
   sscanf(ss,"%s%s%s",tok1,tok2,tok3);
}
   void count()
  {
   ...
   else
  {
   while(read(fp,ch,1)!=0)
   {
    ...
    printf("\n total number of character=%d & words=%d\n",cc,wc);

     else if(strcmp(tok2,"cl")==0)
    printf("\n total number of character=%d & lines=%d\n",cc,lc);

    else if(strcmp(tok2,"wl")==0)
    printf("\n total number of words=%d & lines=%d\n",wc,lc);

  else if(strcmp(tok2,"cwl")==0)
    printf("\n total number of character=%d & words=%d & lines\n",cc,wc,lc);
      }
       }
    int main()
   {
